...
def shortest_path(graph, start, target=None):  # target is optional
    ...
    while heap:
        current_distance, current = heapq.heappop(
            heap)  # get node with smallest distance
...
def format_path_output(distances, paths, start, target=None):
    ...
    for node in targets_to_print:
        ...
        output.append(
            f'{start}-{node} distance: {distances[node]}\nPath: {path_str}')
    return "\n".join(output)


distances, paths = shortest_path(my_graph, 'B')
print(format_path_output(distances, paths, 'B'))
